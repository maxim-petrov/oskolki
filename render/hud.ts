...
/** A flat meter in an ink frame (the paper interface has no highlights on bars). */
function bar(ctx: Ctx2D, x: number, y: number, w: number, h: number, k: number, colors: [string, string, string], back = 'grey4') {
...
export function drawTopBar(ctx: Ctx2D, ui: UI, run: RunState, d: Disp, t: number, pulse: number, incoming: number): string | null {
  ...
  if (d.armor > 0) {
    ...
    x += 10 + text(ctx, `${d.armor}`, x + 10, y + 2, 'cold2', { bold: true }) + 4;
  }
  draw(ctx, getFrame('ui_coin'), x + 4, y + 6);
  x += 10 + text(ctx, `${d.coins}`, x + 10, y + 2, 'gold1', { bold: true }) + 6;
  // Right: buttons.
  let bx = w - 14;
  ...
  if (x < bx - 10) text(ctx, label, bx - 4, y + 2, 'grey1', { align: 'right' });
  return clicked;
}
...
export function drawSkill(ctx: Ctx2D, ui: UI, r: Rect, run: RunState, d: Disp, t: number): boolean {
  ...
  panel(ctx, r.x, r.y, r.w, r.h, { fill: ready ? (hot ? 'vio4' : 'vio5') : hot ? 'rose' : 'paper2', raw: true });
  if (!def) {
  ...
  text(ctx, def.name, tx, r.y + 4, 'ink0', { bold: true });
  const mw = r.x + r.w - 6 - tx;
  bar(ctx, tx, r.y + r.h - 9, mw, 4, d.charge / d.cost, ['vio4', 'vio3', 'vio2'], 'grey4');
  text(ctx, `${d.charge}/${d.cost}`, r.x + r.w - 5, r.y + 4, ready ? 'vio3' : 'grey1', { align: 'right' });
  if (ready && Math.floor(t * 3) % 2 === 0) {
    ctx.fillStyle = hex('vio3');
...
export function drawPockets(ctx: Ctx2D, ui: UI, x: number, y: number, size: number, run: RunState, armed: number): number {
  ...
  run.hero.pockets.forEach((p, k) => {
    ...
    panel(ctx, sx, y, size, size, { fill: armed === k ? 'tile' : hot && p ? 'rose' : 'paper2', raw: true, shadow: 1 });
    if (p) {
      const def = POCKETS[p];
    ...
    if (!L.touch) text(ctx, `${k + 1}`, sx + 3, y + 1, 'grey1');
  });
  return clicked;
}
...
export function drawBossBar(ctx: Ctx2D, name: string, hp: number, max: number, y: number, t: number) {
  ...
  bar(ctx, x, y + 10, w, 6, hp / max, ['red4', 'red2', 'red1'], 'rose2');
  bigText(ctx, name, L.w / 2, y - 4, 'cream', { align: 'center', outline: 'ink0', alpha: 0.95 + Math.sin(t * 3) * 0.05 });
